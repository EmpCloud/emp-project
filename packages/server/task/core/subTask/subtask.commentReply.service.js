...
import subTaskValidate from "./subTask.validate.js";
...
class CommentReplyService {
    async addReply(req, res) {
        ...
        try {
            ...
            const { value, error } = subTaskValidate.postCommentValidation(reply)
            if (error) {
            ...
            const response = await subtaskCommentSchema.findOneAndUpdate({ _id: req.query.commentId }, { $push: { reply: replyedDetailes } }, { returnDocument: 'after' })
    ...
    async updateReply(req, res) {
        ...
        try {
            ...
            const userNameInput = req?.body?.userName;
            const { value, error } = subTaskValidate.postCommentValidation(req.body)
            if (error) {
                if (error) return res.send(Response.validationfailResp("Validation failed", error));
            }
            // const db = await checkCollection(reuse.collectionName.user)
            const response = await subtaskCommentSchema.findOneAndUpdate({ "reply._id": ObjectId(replyId) }, { $set: { "reply.$.comment": reply.comment, "reply.$.userName": userNameInput, "reply.$.isEdited": true } }, {
    ...
    async deleteReply(req, res) {
        ...
        try {
            ...
            let count = 0;
            const commentExist = await subtaskCommentSchema.findOne({ "_id": commentId })
            const isExist = await subtaskCommentSchema.findOne({ "reply._id": ObjectId(replyId) });
            if (commentExist) {
                let response;
                if (userId == commentExist.commentCreator.creatorId) {
                    if (!commentId) {
                        response = await subtaskCommentSchema.updateOne({
            ...
            const response = await subtaskCommentSchema.updateOne({
                "reply._id": ObjectId(replyId)
            }, {
                $pull: {
                    reply: { _id: ObjectId(replyId) }
