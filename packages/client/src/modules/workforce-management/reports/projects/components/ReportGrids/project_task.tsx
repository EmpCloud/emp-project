...
const task_status = ({projectSelected}) => {
  ...
  useEffect(() => {
    ...
    let chart = am4core.create("taskStatus", am4charts.RadarChart);
    chart.logo.disabled = true;

    if (taskStatusDetails) {
        const statusData = taskStatusDetails.statusData;
        const progressData = taskStatusDetails.progressData;
  ...
  return (
    <div className="bg-white shadow-md rounded-lg p-4 w-full">
        <h1 className="xl:text-xl font-semibold text-left dark:text-[#fff] mb-4 relative">Task Status<span className="absolute top-0 -ml-4  inline-flex items-center justify-center mr-2 font-bold leading-none transform translate-x-1/2 -translate-y-1/2 bg-[#0685D7] text-indigo-100 text-sm text-center px-2 py-1 rounded-full dark:bg-[#0685D7] border border-[#0685D7]">{taskStatusDetails?.Total ?? 0}</span></h1>
        <div id="taskStatus" className="w-full h-[400px] md:h-[300px] lg:h-[400px] xl:h-[300px]"></div>
    </div>
  );
};
