...
const subtask_type = ({ clickConfig, handleRemoveGrid, data, key }) => {
    const [typeList, setTypeList] = useState(null);
    const [subtaskByTask, setSubtaskByTask] = useState(null);
    ...
    const filterType = type => {
        if (!subtaskDetails && !type) return false;
        let tempTasks = subtaskDetails?.filter(d => {
            return d.subTaskType == type;
        });
        setSubtaskByTask(tempTasks);
    };

    useEffect(() => {
        filterType(typeList && typeList[0].value);
    }, [subtaskDetails, typeList]);
                ...
                <div className={clickConfig ? 'opacity-30 ' : 'mt-5'}>
                    <div className={`card-d project-stage w-full inline-block align-top `}>
                        <div className='flex justify-between items-center'>
                            <h1 className='heading-medium text-base mb-0 sm:text-xl font-semibold'>Subtask By Type</h1>
                        ...
                        <div className='mt-4 overflow-x-auto relative shadow-md max-h-[260px] overflow-y-auto'>
                            <table className='table-style min-w-[500px] w-full'>
                                ...
                                <tbody className=''>
                                    {subtaskByTask && subtaskByTask.length === 0 && (
                                    ...
                                    {subtaskByTask &&
                                        subtaskByTask.map(function (item, index) {
                                            return (
                                                <tr key={index}>
