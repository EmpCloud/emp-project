...
import { getTaskActivity } from '@WORKFORCE_MODULES/task/api/get';
import React, { useEffect, useState } from 'react';
import { ImCross } from 'react-icons/im';

const task_activity_grid = ({ clickConfig, handleRemoveGrid, data, key, taskList }) => {
    const [taskActivity, setTaskActivity] = useState(null);
    const [selectedOption, setSelectedOption] = useState('');

    const handleGetTaskActivity = id => {
        getTaskActivity(id).then(response => {
            if (response.data.body.status === 'success') {
                setTaskActivity(response.data.body.data);
            }
        });
    };
    ...
    useEffect(() => {
        handleGetTaskActivity(taskList ? taskList[0].value : null);
    }, [taskList]);
    return (
                                    ...
                                    <div className='TaskManageLabel md:w-80 py-2'>
                                        <FloatingOnlySelectfield
                                            ...
                                            onChange={event => {
                                                ...
                                                handleGetTaskActivity(event.target.value);
                                            }}
                                        />
                                    </div>
                                </div>
                                    ...
                                    <table className='w-full text-sm text-left text-gray-500 dark:text-gray-400'>
                                        ...
                                        <tbody>
                                            {taskActivity &&
                                                ...
                                                taskActivity.map(function (data, key) {
                                                    return (
                                                        <>
                                                            <tr key={key} className='bg-white border-b-0 dark:bg-gray-800 dark:border-gray-700'>
                                                                ...
                                                                <th scope='row' className='py-3 px-6 font-medium text-gray-900 whitespace-nowrap dark:text-white'>
                                                                    <div className='flex mb-5 -space-x-4'>
                                                                        <ToolTip className='relative w-[30px] h-[30px] shadow-md rounded-full' message={data.activityCreator.name}>
                                                                            <img
                                                                                key={key}
                                                                                src={data.activityCreator.profilePic}
...
export default task_activity_grid;
