...
const task_category = ({ clickConfig, handleRemoveGrid, data, key }) => {
    ...
    const [taskByCategory, setTaskByCategory] = useState(null);
    ...
    const filterCategory = category => {
        if (!taskDetails && !category) return false;
        let tempTasks = taskDetails?.filter(d => {
            return d.category == category;
        });
        setTaskByCategory(tempTasks);
    };

    useEffect(() => {
        filterCategory(categoryList && categoryList[0]?.value);
    }, [taskDetails, categoryList]);
                ...
                <div className={clickConfig ? 'opacity-30 ' : 'mt-5'}>
                    <div className={`card-d project-stage w-full inline-block align-top `}>
                        <div className='flex justify-between items-center mb-3'>
                            <h1 className='heading-medium text-base mb-0 sm:text-xl font-semibold'>Task Category</h1>
                        ...
                        <div className=' overflow-x-auto relative shadow-md max-h-[280px] overflow-y-auto'>
                            <table className='table-style w-full min-w-[400px]'>
                                ...
                                <tbody className=''>
                                    {taskByCategory && taskByCategory.length === 0 && (
                                    ...
                                    {taskByCategory &&
                                        taskByCategory.map(function (item, index) {
                                            return (
                                                <tr key={index}>
                                                    <td>
                                                    ...
                                                    <td className=''>
                                                        <div className='user-profile-img flex justify-center user-img-group items-center cursor-pointer'>
                                                            ...
                                                            <ToolTip className='relative w-[30px] h-[30px] bg-white shadow-md rounded-full' message={item.taskCreator.firstName} userId={item.taskCreator.id} >
