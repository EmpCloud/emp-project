...
const custom_permission_grid = ({ clickConfig, handleRemoveGrid, data, key }) => {
    ...
    const [permissionDetsils, setPermissionDetails] = useState(null);
    ...
    const membersTableList = [
        ...
        { name: 'Permission', value: 'permission', visible: true },
    ];

    const handleGetAllUser = (condition = '') => {
        getAllUsers(condition).then(response => {
            if (response.data.body.status === 'success') {
                setMemberCount(response.data?.body.data.TotalUserCount);
    ...
    const handleSearchMember = event => {
        searchMember('keyword=' + event.target.value).then(response => {
            if (response.data.body.status === 'success') {
    ...
    const handleGetAllRoles = () => {
        getPermisssionGroup().then(response => {
            if (response.data.body.status === 'success') {
                setPermissionDetails(response.data?.body.data.Permissions);
            }
        });
    };
            ...
            <div className={clickConfig ? 'opacity-30 ' : 'mt-5'}>
                <div className='card p-7 w-full d-flex'>
                    <div className='flex justify-between items-center'>
                        <h3 className='heading-medium'>Custom Permission</h3>
                    ...
                    <div className='overflow-x-auto'>
                        <table className='table-style min-w-[105px] '>
                            <thead>
                                <tr>
                                    {membersTableList &&
...
export default custom_permission_grid;
