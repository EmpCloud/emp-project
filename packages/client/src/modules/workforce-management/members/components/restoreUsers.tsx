...
import { getAllDeletedUsers, getAllRoles, getAllUsers, getEmpUsers, searchMember } from '../api/get';
import TinnySpinner from '../../../../components/TinnySpinner';
import DropDownWithTick from '../../../../components/DropDownWithTick';
import { deleteAllUser, deleteUserById, forceDeleteAllUser, forceDeleteUserById } from '../api/delete';
import DeleteConformation from '../../../../components/DeleteConformation';
import toast from '../../../../components/Toster/index';
import { restoreMemberById, updateMember } from '../api/put';
import SearchInput from '../../../../components/SearchInput';
import { MdKeyboardArrowLeft, MdKeyboardArrowRight } from 'react-icons/md';
import EditMember from './editMember';
import AddBulkUser from './addBulkUser';
import { FaArrowDown, FaArrowUp, FaRedo, FaTrashRestore } from 'react-icons/fa';
...
import RecoverConfirmation from '@COMPONENTS/RecoverConfirmaion';
...
const restoreUsers = ({ startLoading, stopLoading }) => {
    ...
    const handleGetAllDeletedUser = (condition = "") => {
        getAllDeletedUsers(condition).then(response => {
            if (response.data.body.status === 'success') {
    ...
    const handleRecoverUserById = () => {
        restoreMemberById(recoverUserId)
            .then(function (result) {
                if (result.data.body.status == 'success') {
                    handleGetAllDeletedUser('?limit=' + sortTable.limit);
                    toast({
    ...
    const handlePaginationMember = condition => {
        handleGetAllDeletedUser(condition)
    };
    useEffect(()=>{
        handleGetAllDeletedUser('?limit=' + sortTable.limit)
    },[sortTable.limit])
            ...
            {permission && permission.user.delete === false ? <NoAccessCard /> : <>
                <div className='font-inter'>
                    <div className='flex items-center justify-between my-2 mb-4 flex-wrap'>
                        <div className='heading-big relative font-bold mb-0 heading-big text-darkTextColor px-2 py-1'>Restore Users<span className="absolute top-0 -right-3 inline-flex items-center justify-center mr-2 font-bold leading-none transform translate-x-1/2 -translate-y-1/2 bg-[#0685D7] text-indigo-100 text-sm text-center ml-2 px-2 py-1 rounded-full dark:bg-[#0685D7] border border-[#0685D7]">{membersCount}</span></div>
                                                ...
                                                <tr key={key}>
                                                    ...
                                                    <td className='w-[15%] !pt-1 !pb-1'>
                                                        <div className='flex items-center gap-4 text-[17.5px] justify-center'>
                                                            <NewToolTip direction='left' message={'Restore'}>
                                                                <button
                                                                    className='green-link'
                                                                    onClick={() => {
                                                                        setRestoreMessage('Restore Member ' + '"' + data.firstName + ' ' + data.lastName + '"');
                ...
                <RecoverConfirmation open={openRestoreModel} close={handleCloseRestoreModel} message={restoreMessage} onClick={handleRecoverUserById} />
